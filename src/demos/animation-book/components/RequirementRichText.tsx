...
export function RequirementRichText({
  ...
  useEffect(() => {
    ...
    if (editorRef.current.innerHTML !== value.html) editorRef.current.innerHTML = value.html;
  }, [value.html]);
  ...
  const saveSelection = () => {
    ...
    if (editorRef.current.contains(selection.anchorNode)) savedSelectionRef.current = selection.getRangeAt(0).cloneRange();
  };
  ...
  const handleImageChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Keep the previous document when a local image cannot be read.
    }
  };
  ...
  if (readOnly) return <RichTextPreview value={value} />;
    ...
    <div className={`ab-rich-text-editor ${className}`.trim()}>
      <div className="ab-rich-text-toolbar" role="toolbar" aria-label="需求富文本工具栏">
        <label className="ab-rich-text-color" title="文字颜色">
          ...
          <input aria-label="需求文字颜色" type="color" defaultValue="#353e42" onMouseDown={() => saveSelection()} onChange={(event) => runCommand("foreColor", event.target.value)} />
        </label>
...
function RichTextToolButton({
  ...
  return <button type="button" className="ab-rich-text-tool" aria-label={label} title={label} onMouseDown={onMouseDown} onClick={onClick}>{icon}</button>;
}
